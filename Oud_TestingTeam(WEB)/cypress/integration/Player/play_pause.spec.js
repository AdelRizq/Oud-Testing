...
describe("Play/Pause", () => {
  ...
  it("play button", () => {
    ...
    cy.xpath('//*[@id="root"]/div/div[4]/div/div[1]/div/div[2]/div/button[2]')
  ...
  it("pause button", () => {
    cy.wait(1000);
    cy.get(`[data-testid='${self.playerIds.pause}']`)
      .click({ multiple: true, force: true })
      .then((play) => {
  ...
  it(`volumeBar "afterClicking on the play button"`, () => {
    cy.wait(1000);
    cy.get(`[data-testid='${self.playerIds.volumeBar}']`)
      .click("right")
      .then(() => {
        cy.xpath('//*[@id="volume-width"]/div/div').should(
          "have.attr",
          `style`,
          "width: 100%;"
        );
  ...
  it(`progress Bar 'after clicking on the play button once then pause'`, () => {
    ...
    cy.get(`[data-testid='${self.playerIds.progressBar}']`)
      .click("left")
      .then(() => {
        cy.get(`[data-testid='${self.playerIds.currentTime}']`).should(
          "have.text",
          "0.00"
        );
      });
  ...
  it(`progress Bar 'while the play button is clicked'`, () => {
    cy.wait(1000);
    cy.xpath(
      '//*[@id="root"]/div/div[4]/div/div[1]/div/div[2]/div/button[2]'
    ).click();
            ...
            (cTime) => {
              ...
              cy.get(`[data-testid='${self.playerIds.totalTime}']`).then(
                (dTime) => {
                  ...
                  expect(
                    Math.abs(parseInt(currentTime) - parseInt(durationTime / 2))
                  ).lessThan(3);
                }
              );
              //waiting 5 second after clicking play button then check if the time in bar is equal to 5
