...
Cypress.Commands.add("ChangeTheEmail",()=>{
  ...
  cy.get(AccountData.EmailBoxID).clear()
    .type(AccountData.NewEmail)
...
Cypress.Commands.add("CheckOudaccountoverview",()=>{
    ...
    cy.url()
    .should('eq',URLS.OverViewURL)
  
    cy.scrollTo(0, 500)           
    cy.contains('EDIT BROFILE')
      .click({force:true})
    cy.url()
      .should('eq',URLS.EditProfileURL)
...
cy.wait(3000)
if(number==1)
  cy.CheckOudpattern('123@gmail.-a')
else if (number==2)
  cy.CheckOudpattern('123@gmail.a-')
else if (number==3)
  cy.CheckOudpattern('test@gmail')
else
  cy.CheckOudpattern('123@gmail.0a') 
...
Cypress.Commands.add("CheckChangingpassword",()=>{
  ...
  cy.get(AccountData.CurrentPasswordBox).clear().type(AccountData.Password)
  cy.get(AccountData.NewPasswordBox).clear().type(AccountData.WrongNewPassword)
  cy.get(AccountData.ConfirmPasswordBox).clear().type(AccountData.WrongNewPassword)
  // if y
  cy.get(AccountData.SubmitButton).click({force:true})
  cy.contains(AccountData.WrongPasswordMessage)
...
Cypress.Commands.add("HomeOverView",()=>{
  ...
  Cypress.Commands.add("CheckCategories",()=>{
    ...
    for(let i = 1 ; i<=9;i++){
      cy.scrollTo(0,i*750)
      cy.wait(2000)
      cy.xpath(Data.SeeAllXpathFirstPart+i+Data.SeeAllXpathSecondPart).click()
  ...
  Cypress.Commands.add("CheckSearchBar",()=>{
    ...
    cy.wait(2000)
    cy.url().should('eq',URLS.SearchURL)
    cy.get(Data.SearchBarID)
    .type("Test search text")
    cy.wait(2000)
  })
  ...
  Cypress.Commands.add("HomePlaylists",()=>{
      ...
      for (let i = 1 ; i<=10;i++){
        for(let j = 1;j<6;j++){
        cy.xpath(Data.PlaylistFirstHalfOfXpath+i+Data.PlaylistMiddleHalfOfXpath+j+Data.PlaylistlastHalfOfXpath).click({force:true})
        cy.url().should('eq',URLS.SeeAllURL)
        cy.go('back')
        cy.wait(2000)
        }
